...
#include <stdlib.h>
	...
	void processCsvFile(const char csvFilePath[], const char selectedColumns[], const char rowFilterDefinitions[])
	{
		...
		FilterArray filterTarget = getFilterTarget(selectedHeader, rowFilterDefinitions);
		printCharArray(selectedHeader.array);
		printf("\n");
		filter(matrix, filterTarget);
		fclose(file);
	...
	CharArray filterHeader(CharArray header, CharArray splitSelected)
	{
		...
		for (int k = 0; k < splitSelected.length; k++)
		{
			...
			{
				/* printf("-> %s -> %s\n", splitSelected.array[k], header.array[i]); */
			...
			if(match == 0) {
				char msg[] = "Invalid headers: ";
				strcat(msg, splitSelected.array[k]);
				perror(msg);
				exit(EXIT_FAILURE);
			}
		}
			...
			for (int k = 0; k < splitHeader.length; k++)
			{
				if (strstr(splitFilter.array[i], splitHeader.array[k]) != NULL)
				{
					...
					if (strchr(splitFilter.array[i], '<') != NULL)
						...
						char msg[] = "Invalid filter: ";
						strcat(msg, splitFilter.array[i]);	
						perror(msg);
						printf("\n");
						exit(EXIT_FAILURE);
					}
